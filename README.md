...
For this project, a mini honeynet was set up on Microsoft Azure, and various log sources were integrated into a Log Analytics workspace. To enable effective threat detection and response, Microsoft Sentinel was utilized to trigger alerts and generate incidents based on the logs ingested. Furthermore, metrics were collected in the initial insecure environment and subsequently after implementing security measures. Notably, the implementation of security controls resulted in a significant reduction in the number of security events and incidents, highlighting the efficacy of the measures deployed.

It is important to consider that if the network resources were heavily utilized by regular users, it is possible that a greater number of security events and alerts would have been generated within the 24-hour period following the implementation of the security controls. This observation emphasizes the potential impact of user activity on the generation of security-related events and underlines the importance of accounting for user behavior and network usage patterns when evaluating security measures.
